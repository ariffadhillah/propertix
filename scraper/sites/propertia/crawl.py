...
def _with_query_param(url: str, key: str, value: str) -> str:
    """
    Safely add/replace a query parameter without duplicating keys (e.g. page=1&page=2).
    Works whether start_url already has query params or not.
    """
    u = urlsplit(url)
...
def iter_list_items(
    ...
    IMPORTANT:
      - start_url may or may not already contain `page=`; we always overwrite it safely.
    """
    ...
    while page <= max_pages:
        url = _with_query_param(start_url, "page", str(page))
        print(f"[BHI] Crawling page {page}: {url}")
