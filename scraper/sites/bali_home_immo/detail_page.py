...
#       - If sale categories exist (freehold/leasehold) => offer_category = "sale"
#         even if monthly/yearly also exist (because it's still a sale listing with extra rent option).
#       - Else if only rent categories exist => offer_category = "rent"
#       - rent_period only meaningful when offer_category == "rent"; otherwise "unknown"
#     """
#     # defaults from URL (fallback only)
...
#     has_sale = ("freehold" in cats) or ("leasehold" in cats)
#     has_rent = ("monthly" in cats) or ("yearly" in cats)

#     # --- offer_category (strongest)
#     if has_sale:
#         offer = "sale"
#     elif has_rent:
#         offer = "rent"
#     else:
#         offer = offer  # fallback from URL

#     # --- tenure_type (only from sale cats if present)
#     if "freehold" in cats:
#         tenure = "freehold"
#     elif "leasehold" in cats:
#         tenure = "leasehold"
#     else:
#         tenure = tenure  # fallback from URL

#     # --- rent_period (ONLY if offer is rent)
#     if offer == "rent":
#         if "monthly" in cats:
#             rent_period = "month"
#         elif "yearly" in cats:
...
#             "contact_links_raw": broker.get("contact_links_raw") or {
#                 "mailto": [], "tel": [], "whatsapp": [], "messenger": [], "form": [], "other": []
#             },
#         },
#         "raw": {
...
# scraper/sites/bali_home_immo/detail_page.py
...
def _parse_sqm(val: str) -> float | None:
    """Parse '200 m²' / '1.200 m2' into float sqm."""
    if not val:
        return None
    s = val.replace("\xa0", " ").strip().lower()
    s = s.replace("m²", "").replace("sqm", "").replace("m2", "")
...
def parse_footer_broker(soup: BeautifulSoup, page_url: str) -> dict:
    ...
    text = footer.get_text(" ", strip=True)
    m = re.search(
        r"Copyright\s*©\s*\d{4}\.?\s*([A-Za-z0-9\s]+?)\s*All Rights Reserved",
        text,
        re.I,
    )
    if m:
    ...
    for k in list(contact_links_raw.keys()):
        contact_links_raw[k] = _dedupe_preserve_order(contact_links_raw[k])
...
# ============================================================
# URL taxonomy + breadcrumb + side location
# ============================================================
...
def extract_table_kv(container: Any) -> dict[str, str]:
    """
    container: element div#list-xxx-{cat}
    supports 2 formats:
    - 3 td: key, ':', value
...
def extract_prices(soup: BeautifulSoup) -> list[dict]:
    ...
    for sp in spans:
        raw_price = (sp.get("data-price") or "").strip()
        category = (sp.get("data-price-category") or "").strip().lower()
        ...
        out.append(
            {
                ...
                "amount": amount,
                "period": _map_period(category),
                "category": category,  # internal only; removed when writing listing.price(s)
            }
        )
    ...
    deduped: list[dict] = []
    for p in out:
        k = (p.get("amount"), p.get("period"), p.get("category"))
        if k in seen:
            continue
        seen.add(k)
        deduped.append(p)

    # stable order (prevents random "updated" hash)
    period_order = {"one_time": 0, "year": 1, "month": 2}
    deduped.sort(key=lambda x: (period_order.get(x.get("period") or "", 99), float(x.get("amount") or 0)))
...
def choose_primary_price(prices: list[dict], source_url: str) -> dict | None:
    ...
    def find_cat(cat: str) -> dict | None:
        for p in prices:
            if p.get("category") == cat:
                return p
        return None
...
# ============================================================
# Client taxonomy inference (offer/tenure/rent_period)
# ============================================================
...
def infer_client_taxonomy(taxonomy_from_url: dict, price_categories: list[str]) -> dict:
    ...
    Rules (MVP, deterministic):
      - If sale categories exist (freehold/leasehold) => offer_category = "sale"
        even if monthly/yearly also exist.
      - Else if only rent categories exist => offer_category = "rent"
      - rent_period only meaningful when offer_category == "rent"; otherwise "unknown"
    """
    offer = coerce_offer_category(taxonomy_from_url.get("intent"))
...
def choose_primary_category(
    *,
    ...
    price_categories: list[str] | None,
) -> str:
    """
    Decide which category to use to pick table/specs:
      - sale  -> prefer tenure_type (freehold/leasehold)
      - rent  -> prefer rent_period (yearly/monthly)
      - fallback -> first known category present
    """
    offer = (offer_category or "").lower().strip()
    tenure = (tenure_type or "").lower().strip()
    rentp = (rent_period or "").lower().strip()
    cats = [c.lower().strip() for c in (price_categories or []) if c]
    ...
    if offer == "rent":
        if rentp in ("year", "yearly", "annual"):
            return "yearly"
        if rentp in ("month", "monthly"):
            return "monthly"

    for c in ("freehold", "leasehold", "yearly", "monthly"):
        if c in cats:
            return c

    return "leasehold" if "leasehold" in cats else (cats[0] if cats else "freehold")
...
def parse_detail_page(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    preferred_cat = choose_primary_category(
        offer_category=client_tax.get("offer_category"),
        tenure_type=client_tax.get("tenure_type"),
        rent_period=client_tax.get("rent_period"),
        price_categories=price_categories,
    )
    ...
    gen_cat, general = pick_category_dict(general_all, preferred_cat)
    in_cat, indoor = pick_category_dict(indoor_all, preferred_cat)
    out_cat, outdoor = pick_category_dict(outdoor_all, preferred_cat)
